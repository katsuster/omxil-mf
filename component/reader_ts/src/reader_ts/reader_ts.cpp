...
reader_ts::worker_main::worker_main(reader_ts *c)
	: component_worker(c), comp(c)
{
}

reader_ts::worker_main::~worker_main()
{
}
...
reader_ts::reader_ts(OMX_COMPONENTTYPE *c, const char *cname) 
	: component(c, cname),
...
OMX_U32 reader_ts::get_video_ports()
{
	return 1;
}

OMX_U32 reader_ts::get_video_start_port()
{
	return 2;
}
...
reader_ts *reader_ts::get_instance(OMX_HANDLETYPE hComponent)
{
	OMX_COMPONENTTYPE *omx_comp = (OMX_COMPONENTTYPE *) hComponent;
	reader_ts *comp = (reader_ts *) omx_comp->pComponentPrivate;
