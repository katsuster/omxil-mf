...
class comp_test_disable_port : public omxil_comp {
public:
	typedef omxil_comp super;

	comp_test_disable_port(const char *comp_name)
		: omxil_comp(comp_name)
	{
		//do nothing
	}

	virtual ~comp_test_disable_port()
	{
		//do nothing
	}
	...
	virtual OMX_ERRORTYPE EmptyBufferDone(OMX_HANDLETYPE hComponent, OMX_PTR pAppData, OMX_BUFFERHEADERTYPE* pBuffer)
	{
		...
		snprintf(lb, sizeof(lb), "empty %3d: ", cnt_in);
		buf += lb;
		for (int i = 0; i < 16; i++) {
			snprintf(lb, sizeof(lb), "%02x ", pBuffer->pBuffer[i]);
			buf += lb;
		}
		printf("%s\n", buf.c_str());
	...
	virtual OMX_ERRORTYPE FillBufferDone(OMX_HANDLETYPE hComponent, OMX_PTR pAppData, OMX_BUFFERHEADERTYPE* pBuffer)
	{
		...
		snprintf(lb, sizeof(lb), "fill  %3d: ", cnt_out);
		buf += lb;
		for (int i = 0; i < 16; i++) {
			snprintf(lb, sizeof(lb), "%02x ", pBuffer->pBuffer[i]);
			buf += lb;
...
int main(int argc, char *argv[])
{
	...
	comp = new comp_test_disable_port(arg_comp);
	...
	fut_out = std::async(std::launch::async,
		...
		for (int i = 0; i < maxcnt; i++) {
			...
			if (result != OMX_ErrorNone) {
				fprintf(stderr, "FillThisBuffer(%d) part 1 failed.\n",
					(int)pnum_out);
				return -1;
			}
		}
	...
	//Send disable port command
	result = comp->SendCommand(OMX_CommandPortDisable, pnum_out, 0);
	if (result != OMX_ErrorNone) {
		fprintf(stderr, "OMX_SendCommand(CommandPortDisable, out) failed.\n");
		goto err_out2;
	}
	...
	for (auto it = buf_out.begin(); it != buf_out.end(); it++) {
		...
		comp->unregister_buffer(pnum_out, *it);
	...
	printf("wait for Disable(out) command...\n");
	comp->wait_command_completed(OMX_CommandPortDisable, pnum_out);
	printf("wait for Disable(out) command... Done!\n");


	//Send enable port command
	result = comp->SendCommand(OMX_CommandPortEnable, pnum_out, 0);
	if (result != OMX_ErrorNone) {
		fprintf(stderr, "OMX_SendCommand(CommandPortEnable, out) failed.\n");
		goto err_out2;
	}
	...
	printf("wait for Enable(out) command...\n");
	comp->wait_command_completed(OMX_CommandPortEnable, pnum_out);
	printf("wait for Enable(out) command... Done!\n");


	//FillThisBuffer part 2
	fut_out = std::async(std::launch::async,
		[&] (int maxcnt) -> int {
		for (int i = 0; i < maxcnt; i++) {
			OMX_BUFFERHEADERTYPE *buf;
			...
			result = comp->FillThisBuffer(buf);
			if (result != OMX_ErrorNone) {
				fprintf(stderr, "FillThisBuffer(%d) part 2 failed.\n",
					(int)pnum_out);
				return -1;
			}
		}
	...
	//Get Empty result and Fill result part 2
	ret_in = fut_in.get();
	if (ret_in != 0) {
		fprintf(stderr, "EmptyThisBuffer(%d) failed.\n",
			(int)pnum_in);
