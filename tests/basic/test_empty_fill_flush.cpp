...
class comp_test_empty_fill_flush : public omxil_comp {
public:
	typedef omxil_comp super;

	comp_test_empty_fill_flush(const char *comp_name)
		: omxil_comp(comp_name)
	{
		//do nothing
	}

	virtual ~comp_test_empty_fill_flush()
	{
		//do nothing
	}
...
{
	const char *arg_comp;
	comp_test_empty_fill_flush *comp;
	...
	auto lambda_out = [&] (int maxcnt) -> int {
		for (int i = 0; i < maxcnt; i++) {
			OMX_BUFFERHEADERTYPE *buf;
			OMX_ERRORTYPE result;
	...
	comp = new comp_test_empty_fill_flush(arg_comp);
	...
	//FIXME: Component cannot execute after flush...
	{
		...
		//Send flush command
		result = comp->SendCommand(OMX_CommandFlush, OMX_ALL, 0);
		if (result != OMX_ErrorNone) {
			fprintf(stderr, "OMX_SendCommand(CommandFlush, OMX_ALL) failed.\n");
			goto err_out2;
		}
...

	{
		...
		fut_out = std::async(std::launch::async, lambda_out, 50 + 1);
		ret_in = fut_in.get();
		if (ret_in != 0) {
			fprintf(stderr, "EmptyThisBuffer(%d) failed.\n",
				(int)pnum_in);
